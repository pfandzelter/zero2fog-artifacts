...
fs.writeFileSync(
  filename,
  "camera_type,wgw_type,fctry_datacenter_type,office_datacenter_type,cloud_type,prod_cntrl_type,cfd_map,adapt_map,predict_map,aggregate_map,generatedashboard_map,proc_time,proc_cost,tx_time,tx_cost,full_time,full_cost,existsCoherentFlow\n"
);
...
let cfd = ["camera", "prod_cntrl", "wgw", "fctry_datacenter"];
let adapt = ["sensor", "pack_cntrl", "wgw", "fctry_datacenter"];
let predict = ["fctry_datacenter", "office_datacenter", "cloud"];
let aggregate = [
  "camera",
  "prod_cntrl",
  "sensor",
...
let generatedashboard = ["fctry_datacenter", "office_datacenter", "cloud"];
...
infrastructureOptions.forEach((infrastructure, i) => {
  ...
  let thismodel = JSON.parse(JSON.stringify(require("./model.json")));
  ...
  mappingOptions.forEach((mapping, i) => {
    ...
    console.log(counter);
    // add mapping
    evaluator.setCurrentlySelectedModuleToModuleWithId("cfd");
    evaluator.assignCurrentlySelectedModuleToNodeWithId(mapping.c);

    evaluator.setCurrentlySelectedModuleToModuleWithId("adapt");
    evaluator.assignCurrentlySelectedModuleToNodeWithId(mapping.a);

    evaluator.setCurrentlySelectedModuleToModuleWithId("predict");
    evaluator.assignCurrentlySelectedModuleToNodeWithId(mapping.p);

    evaluator.setCurrentlySelectedModuleToModuleWithId("aggregate");
    evaluator.assignCurrentlySelectedModuleToNodeWithId(mapping.ag);

    evaluator.setCurrentlySelectedModuleToModuleWithId("generatedashboard");
    evaluator.assignCurrentlySelectedModuleToNodeWithId(mapping.g);
    ...
    resultsString += ",";
    resultsString += mapping.a;
    resultsString += ",";
    resultsString += mapping.p;
    resultsString += ",";
    resultsString += mapping.ag;
    resultsString += ",";
    resultsString += mapping.g;
    resultsString += ",";
    ...
    if (
      existsCoherentFlow &&
      procTime + txTime != Number.POSITIVE_INFINITY &&
      procCost + txCost != Number.POSITIVE_INFINITY
    ) {
      let name = "";
      ...
      fs.writeFileSync(
        "./results/" + name + ".json",
        evaluator.getAllDataRepresentation()
      );
    }
  });
});
